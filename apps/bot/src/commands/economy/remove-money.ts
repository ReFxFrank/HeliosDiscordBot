...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Remove currency from a member’s balance (admin).')
  ...
  async execute(interaction, ctx) {
    ...
    await interaction.reply({
      embeds: [
        brandedEmbed({
          kind: 'success',
          description: `✅ Removed ${formatMoney(amount, config)} from ${target}'s ${where}. New ${where}: ${formatMoney(next, config)}.`,
        }),
      ],
    });
  },
